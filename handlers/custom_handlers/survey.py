...
def get_city(message: Message) -> None:
    bot.send_message(message.from_user.id, 'Отправь свой номер нажав на кнопку',
...
def get_contact(message: Message) -> None:
    if message.content_type == 'contact':
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            ...
            text = f'Спасибо за предоставленную Вами информацию: \n ' \
                   f'Имя: {data["name"]}, \n возраст: {data["age"]}, \n Город: {data["city"]}, ' \
                   f'\nСтрана {data["country"]}, \n Номер телефона: {data["phone_number"]}.'
            bot.send_message(message.from_user.id, text)
    else:
        bot.send_message(message.from_user.id, 'Чтобы отправить номер телефона нажми на кнопку.')